...
/*	CKEDITOR.replace("con", {
   ...
   //효과 - 선택버튼 클릭시
	...
	$(".middle_table").on("click", "td", function()  {
		...
		console.log("효과명 : " + popupEffectName);
		console.log("점수 : " + popupEffectScore);
...
function revTitleList(revTitleList, getRevCon) {
	...
	for(data of revTitleList) {
		...
		if(getRevCon != null){
			...
			console.log($("#con").val() + '1232141');
			console.log(getRevCon.CON + '   이건?');
			console.log('asdasdasdqwweqewqe2e');
		}else{
			conData += "<textarea rows=\"10\" cols=\"30\" id=\"con\">" + "" + "</textarea>";
			console.log('ra');
			console.log($("#con").val() + '121');
		}

...
$(document).ready(function(){
	...
	$(document).on("click", ".commentUpdateBtn", function() {
		if($(this).val() == "수 정") {
			$(this).val("수정완료");
			console.log($(this).parent().parent().children().eq(0).children().val());
	...
	$(document).on("click", ".commentDeleteBtn", function() {
		...
		$(document).on("click", ".reviewList .spreadBtn",function() {
			...
			if($(this).val() =="펼 치 기") {
				...
				//alert($(this).parent().parent().attr("no"));
				...
				$(".editor_border").hide();
				$(".paging_area2").hide();
				//$(".main_border").css("height" , "");
				//$(".listBtn").css("margin-top" , "");
			}
		});
...
function drawList(list) {
	...
	//console.log("effect = " + effectNmArr);
...
function reloadGraph2() {
	...
	$.ajax({
		...
		success : function(res) {
			console.log("??????????/");
			console.log("기타 부작용1");
			console.log(res.etcseffectDetailNm);
			console.log("기타 부작용2");
			console.log(res.etcseffectDetailScore);
...
function reloadRevTable(revDetailEffect, revDetailSEffect, revDetailEffectEtc, revDetailSEffectEtc) {
	...
	/*
	for(var row=0;row<effectArr1.length;row++) {
		effectSum1 += parseInt(effectArr1[row]);
		console.log("점수 = " + effectArr1[row]);
		console.log(effectSum1);
	}
*/
}
...
function drawPaging2(pd2) {
	...
	console.log(pd2.startP);
	console.log(pd2.endP);
	console.log(pd2.maxP);
